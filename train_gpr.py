...
def evaluate(model, likelihood, file_path, device):
    ...
    positions, atomic_numbers, _, (target_energy, _) = next(iter(eval_dataloader))
...
def train(model_name, file_path, device):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--file_path', type=str, required=True, help='Path to the root directory')
    ...
    train(args.model_name, args.file_path, torch.device(args.device))
    # python train_gpr.py --model_name gpr_PeriodicKernel_lr_0.1_patience_1000 --file_path /home/sait2024/Project/uncertainty/ --device cuda 
