...
def train(alpha, file_path, model_name, device):
    ...
    # prepare train data
    train_x = torch.load(f"/home/sait2024/Project/uncertainty/split_data/train_1200_x_{idx}.pt").to(device)
    train_y = torch.load(f"/home/sait2024/Project/uncertainty/split_data/train_1200_y_{idx}.pt").to(device)
    ...
    test_x = torch.cat((positions.view(len(test_dataset), -1), atomic_numbers), dim=1).to(device)  # torch.Size([16, 384])
    test_y = target_energy.to(device)  # torch.Size([1400])
    
    # setup model    
    likelihood = gpytorch.likelihoods.GaussianLikelihood().to(device)
    model = GPRegressionModel(idx, train_x, train_y, likelihood).to(device)
...
if __name__ == "__main__":
    ...
    model_name = f"gpr_RQ_1200_{idx}_lr_0.1_patience_1000"
    device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
    # device = torch.device("cpu")
    
    train(idx, file_path, model_name, device)
