...
class StudentTProcessModel(ExactGP):
    def __init__(self, train_x, train_y, likelihood, df=3):
        super(StudentTProcessModel, self).__init__(train_x, train_y, likelihood)
    ...
    def forward(self, x):
        ...
        student_t_dist = MultivariateStudentT(self.df, mean_x, covar_x)
        # student_t_dist = distributions.StudentT(base_dist.mean, base_dist.covariance_matrix, df=self.df)
        return student_t_dist
...
class MultivariateStudentT(TDistribution):
    ...
    def __init__(self, df, loc, scale, validate_args=None):
        ...
        batch_shape = torch.broadcast_shapes(scale.shape[0], loc.shape[0])
        ...
        event_shape = torch.tensor(self.loc.shape[-1:]) # torch.Size([1400])
